...
// Применение стилей к разделителям (divider) на основе атрибутов из ссылки
...
// Вспомогательная функция для применения стилей к разделителю
...
// Вспомогательная функция для получения вычисленного цвета (используется только adjustDividers)
...
// Функция для автоматического изменения цветов разделителей внутри блоков callout в зависимости от цвета фона callout
// В текущей реализации Notion, разделители внутри callout блоков не адаптируются к цвету фона, что может привести к плохой видимости на некоторых цветах
// Эта функция пытается решить эту проблему, устанавливая цвет разделителей в зависимости от цвета фона callout блока.
// TODO: Деактивирована, т.к. вызывается слишком часто и вызывает лаги. Некорректно определяет цвет фона
