...
// Функция обработки существующих цветных и стилизованных ссылок
function processTextStyleLinks(container = document) {
    ...
    links.forEach((link, index) => {
        ...
        if (colorMatch) {
            console.log(`Processing legacy color link ${index + 1}: ${href}`);
            link.style.color = `#${colorMatch[1]}`;
            ...
            console.log(`Processed legacy color link ${index + 1} with color: #${colorMatch[1]}`);
            return;
        }
        ...
        console.log(`Processing styled link ${index + 1} with attributes:`, attributes);
        ...
        // Process attributes
        // [0] type: 0=text, 1=info, 2+=text
        if (attributes[0]) {
            const type = parseInt(attributes[0], 16);
            if (type === 1) {
        ...
        // [3] backgroundColor Uptown FunkColor: #xxxxxx
        if (attributes[3]) {
            const bgColor = attributes[3].match(/[0-9a-fA-F]{6}/)?.[0];
            if (bgColor) link.style.backgroundColor = `#${bgColor}`;
        }

        // [4] textDecoration: 0=none, 1=underline, 2=overline, 3=line-through, 4=underline+overline, 5+=none
        if (attributes[4]) {
        ...
        // [6] fontStyle: 0=normal, 1=italic, 2=oblique, 3+=normal
        if (attributes[6]) {
        ...
        // [7] fontWeight: 0=normal, 1=bold, 2=lighter, 3=bolder, 4+=normal
        if (attributes[7]) {
        ...
        // [10] whiteSpace: 0=normal, 1=nowrap, 2=pre, 3+=normal
        if (attributes[10]) {
        ...
        // [11] direction: 0=ltr, 1=rtl, 2+=ltr
        if (attributes[11]) {
            const dir = parseInt(attributes[11], 16);
            link.style.direction = dir === 1 ? 'rtl' : 'ltr';
        }

        // [12] verticalAlign: 0=baseline, 1=sub, 2=super, 3=middle, 4=top, 5=bottom, 6+=baseline
        if (attributes[12]) {
